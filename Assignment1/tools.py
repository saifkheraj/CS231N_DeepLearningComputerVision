    ...
    for f in d:
        ...
        if f[2] == 'i' and f[3].isdigit():
            ...
            if len(zfile) != 0:  # unrar the contents in the folder...
                ...
                os.chdir(os.path.pardir)
            zfile = get_files(sdir, '.tar.gz')
            if len(zfile) != 0:  # unrar the contents in the folder...
                os.chdir(sdir)
                tfile = generate_valid_filename(zfile[0])
                shutil.move(zfile[0], tfile)
                os.system('tar -xvf ' + generate_valid_filename(zfile[0]))
                os.chdir(os.path.pardir)
...
def shuffle_data(X,Y):
    """
        Shuffles the training data (X) and labels Y

        Input:
            X= m x n training matrix of m examples
