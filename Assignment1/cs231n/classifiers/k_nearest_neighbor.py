...
class KNearestNeighbor(object):
  ...
  def compute_distances_one_loop(self, X):
    ...
    for i in xrange(num_test):
      ...
      #######################################################################
      if(i%100==0): print i
      diff=np.tile(X[i,:],(num_train,1))-self.X_train
  ...
  def compute_distances_no_loops(self, X):
    ...
    #L2 distance can be expanded to find vectorized implementation
    #||x-y||^2 =x^2-2XY+Y^2
    #=X^2 - 2(X.T)(Y) +Y^2
    #=||X|| - 2.(X.T).(Y) +||Y||
    X1=np.sum(np.square(X), axis=1) 
    X2=(np.sum(np.square(self.X_train), axis=1))
    xty=2.0 * (np.dot(X, self.X_train.T))
    d=X1+(X2-xty).T
    dists=np.sqrt(d.T)
    #########################################################################
    #                         END OF YOUR CODE                              #
